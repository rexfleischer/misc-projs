 ...
 * @author REx
 */
public class Compare_GreaterThanEqual implements ICompilingOperation
{
            ...
            throws FledSyntaxException
    {
        ...
        {
            writer.writeOperation(ByteCodeOperation.GT_U);
        ...
        else
        {
            switch(left.type)
            {
                case INTEGER:
                    ...
                    {
                         writer.writeOperation(ByteCodeOperation.GTE_I);
                        environment.push(new Value(Value.ValueType.BOOLEAN, null));
                    }
                    else if (right.type == Value.ValueType.DOUBLE)
                    {
                        writer.writeConversion(ByteCodeOperation.I_TO_D, 1);
                        writer.writeOperation(ByteCodeOperation.GTE_D);
                        environment.push(new Value(Value.ValueType.BOOLEAN, null));
                    }
                ...
                case STRING:
                    ...
                    {
                        writer.writeOperation(ByteCodeOperation.GTE_S);
                        environment.push(new Value(Value.ValueType.BOOLEAN, null));
                    }
