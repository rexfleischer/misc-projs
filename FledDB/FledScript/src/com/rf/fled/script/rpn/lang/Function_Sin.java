 ...
 * @author REx
 */
public class Function_Sin implements RPNFunction 
{
    @Override
    public TokenPair compute(TokenPair[] params, RPNExecutionState state) 
    {
        ...
        {
            return new TokenPair(null, TokenType.INTEGER, Math.sin(((Integer) left.value)));
        }
        if (left.type == TokenType.DOUBLE)
        {
            return new TokenPair(null, TokenType.DOUBLE, Math.sin(((Double) left.value)));
        }
