 ...
 * @author REx
 */
public class Function_Cos implements RPNFunction 
{
    @Override
    public TokenPair compute(TokenPair[] params, RPNExecutionState state) 
    {
        ...
        {
            return new TokenPair(null, TokenType.INTEGER, Math.cos(((Integer) left.value)));
        }
        if (left.type == TokenType.DOUBLE)
        {
            return new TokenPair(null, TokenType.DOUBLE, Math.cos(((Double) left.value)));
        }
