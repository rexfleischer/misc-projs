 ...
 * be seen as any of the avaliable TokenType enums.
 * 
...
public class Tokenizer implements Serializable
{
     ...
     */
    private FunctionSet specific;
    
    public Tokenizer(FunctionSet specific)
    {
    ...
    public ArrayList<TokenPair> tokenize(String expression) 
            throws FledSyntaxException
    {
        ...
        for(int i = 0; i < expression.length(); i++)
        {
            ...
            else if (isParathesis(character))
            {
                ...
                {
                    TokenPair token = result.get(result.size() - 1);
                    Function function = (specific != null) ?
                        specific.getFunctionSafe(token.name) : null;
            ...
            int end)
    {
        switch(type)
        {
            ...
            case SEPERATOR:
            {
                ...
                result.add(new TokenPair(character, type, null));
                break;
            }
            ...
            case STRING:
            {
                ...
                result.add(new TokenPair(
                        expression.substring(start, start + 1), 
                        type, 
                        characters));
                break;
            }
            ...
            case INTEGER:
            {
                ...
                result.add(new TokenPair(characters, type, value));
                break;
            }
            ...
            case FUNCTION:
            {
                String characters = expression.substring(start, end);
                Function function = (specific != null) ?
                        specific.getFunctionSafe(characters) : null;
                if (function != null)
                {
                    type = function.getType();
                }
                result.add(new TokenPair(characters, type, null));
                break;
            }
            ...
            case VARIABLE:
            {
                ...
                if (characters.equals("true") || characters.equals("false"))
                {
                    ...
                    result.add(new TokenPair(characters, TokenType.BOOLEAN, value));
                }
                else if (characters.equals("null"))
                {
                    result.add(new TokenPair(characters, TokenType.NULL, null));
                }
                else
                {
                    Function function = (specific != null) ?
                    ...
                    if (function != null)
                    {
                        result.add(new TokenPair(characters, function.getType(), null));
                    }
                    else
                    {
                        result.add(new TokenPair(characters, type, null));
