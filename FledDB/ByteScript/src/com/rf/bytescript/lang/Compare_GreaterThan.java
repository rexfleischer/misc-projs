 ...
 * @author REx
 */
public class Compare_GreaterThan implements ICompilingOperation
{
        ...
        else
        {
            switch(left.type)
            {
                case INTEGER:
                    ...
                    {
                         writer.writeOperation(ByteCodeOperation.GT_I);
                        environment.push(new Value(ValueType.BOOLEAN, null));
                    }
                    else if (right.type == ValueType.DOUBLE)
                    {
                        writer.writeConversion(ByteCodeOperation.I_TO_D, 1);
                        writer.writeOperation(ByteCodeOperation.GT_D);
                        environment.push(new Value(ValueType.BOOLEAN, null));
                    }
                ...
                case STRING:
                    ...
                    {
                        writer.writeOperation(ByteCodeOperation.GT_S);
                        environment.push(new Value(ValueType.BOOLEAN, null));
                    }
