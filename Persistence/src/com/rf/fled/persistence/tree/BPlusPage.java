 ...
 * @author REx
 */
public final class BPlusPage implements ITransactionalRecord
{
    protected Object[] objectCache;
    
    protected RecordFile bytes;
    ...
    protected ReentrantReadWriteLock locking;
    ...
    boolean update(long id, Object record) 
            throws FledPersistenceException, IOException 
    {
        int index = findFirstLessOrEqualChild(id);
        if (isLeaf)
        ...
        {
            return bplustree.loadPage(getChildId(index)).update(id, record);
        }
    }

    BPlusInsertResult insert(long key, Object record, boolean replace) 
            throws IOException, FledPersistenceException
    {
        ...
        {
            long id = getChildId(index);
            if (id == 0)
            {
                throw new FledPersistenceException("child id cannot be 0");
            }
            BPlusPage child = bplustree.loadPage(id);
    ...
    BPlusDeleteResult delete(long key) throws IOException, FledPersistenceException
    {
        BPlusDeleteResult result = null;
        int half = bplustree.maxRecords >> 1;
        int index = findFirstLessOrEqualChild(key);

        if (isLeaf)
        {
            result = new BPlusDeleteResult();
            if (keys[index] != key)
            {
            ...
            if (result.underflow)
            {
                if (index < half)
                {
                    ...
                    else
                    {
                        ...
                        // @TODO optimization: check the other side
                        if (brother.compacityUsed() != half)
                        {
                            ...
                            throw new FledPersistenceException(
                                    "brother has less than half compacity: "
    ...
    protected void commitMeta()
    {
        ByteArray array = new ByteArray(22 + 8 * bytes.compacity() + 1);
        ...
        bytes.setMeta(array.copyUsedBytes());
    }
    
    protected void setMeta(byte[] data)
    {
