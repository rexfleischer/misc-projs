...
public class Tokenizer implements Serializable
{
     ...
     */
    private OrderedNamedValueArray<TypePrecedencePair> specific;
    
    public Tokenizer(OrderedNamedValueArray<TypePrecedencePair> specific)
    {
        if (specific == null)
        {
            throw new NullPointerException("specific");
        }
    ...
    public Tokenizer()
    {
        this(new OrderedNamedValueArray<TypePrecedencePair>(5));
    }
        ...
        for(int i = 0; i < expression.length(); i++)
        {
            ...
            else if (isOperator(character))
            {
                ...
                else
                {
                     ...
                     * first, we are assuming that any set of operator strings
                     * ending in '=' is a single operator (like ==, &=, :=, +=,
                     * -=, ect). also, any operator string that has the same 
                     * operator two times in a row is also considered an 
                     * operator (like &&, ||, >>, ect).
                     */
                    char markChar = expression.charAt(mark);
                    if (character == '=' || character == markChar)
                    {
                        /**
            ...
            else if (isParathesis(character))
            {
                         ...
                         character == '(')
                {
                    ...
                    TypePrecedencePair special = specific.getValueSafe(token.getValueAsString());

                    if (special != null)
                    {
                        token.type = special.type;
                    }
                    else
                    {
            ...
            else if (isBracket(character))
            {
                ...
                flush(result, TokenType.BRACKET, expression, i, i);
                dirty = false;
                mark = i + 1;
                curr = null;
            }
            ...
            int end)
    {
        switch(type)
        {
            ...
            case SEPERATOR:
            {
                ...
                result.add(new Token(type, character));
                break;
            }
            ...
            case STRING:
            {
                ...
                result.add(new Token(type, characters));
                break;
            }
            ...
            {
                String characters = expression.substring(start, end);
                TypePrecedencePair special = specific.getValueSafe(characters);
                if (special != null)
                {
                    type = special.type;
                }
            ...
            case VARIABLE:
            {
                ...
                if (characters.equals("true") || characters.equals("false"))
                {
                    ...
                    result.add(new Token(TokenType.BOOLEAN, value));
                }
                ...
                else
                {
                    ...
                    if (special != null)
                    {
                        result.add(new Token(special.type, characters));
                    }
                    else
                    {
                        result.add(new Token(type, characters));
